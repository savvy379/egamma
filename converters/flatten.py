...
def main ():
    ...
    for path in args.paths:
        ...
        for irow in range(data_.shape[0]):
            if True in map(is_bad, data_[irow].tolist()):
                print "Row {}/{} is bad!".format(irow, data_.shape[0])
                pass
            pass
...
def convert_flatten (data):
    ...
    # Fields to read
    scalars = ['pt', 'et', 'eta', 'phi', 'type', 'origin', 'truthPdgId', 'LHLoose', 'LHMedium', 'LHTight', 'LHValue', 'Loose', 'Medium', 'Tight']  # (num_cands,)
    vectors = ['energy', 'gain', 'provenance', 'sampling', 'time', 'x', 'y', 'z', 'dx', 'dy', 'dz', 'eta', 'phi', 'etasize', 'phisize']  # (num_cands, num_cells)
